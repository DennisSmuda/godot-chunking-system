...
const _TileChunk = preload("res://scenes/tile_chunk.tscn")
...
func _ready() -> void:
	...
	Events.player_move.connect(_on_player_move)

	# update update_timer
	update_timer = Timer.new()
	update_timer.timeout.connect(_update_world)
...
func load_done(x: int, y: int, chunk: TileChunk, _thread: Thread) -> void:
	var key = str(x) + "," + str(y)
	add_child(chunk)
	chunks[key] = chunk
...
func _on_player_move(_position: Vector2) -> void:
...
func create_chunk(x, y) -> TileChunk:
	var new_chunk = _TileChunk.instantiate()
